...
class SubConsController extends Controller {
	...
	public function subcons(Request $request){

		$data = array(
			'subConCode'=>$request->input('subConCode'),
		);


		$subcons = DB::table('subcons as subcon')
              ->select( 
                        'subcon.subcon_code',
                        'subcon.subcon_name'
              );
		...
		if ($data['subConCode']){
			$subcons = $subcons->where('subcon.subcon_code', $data['subConCode']);
		}
